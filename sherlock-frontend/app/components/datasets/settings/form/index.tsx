...
const Form = () => {
  ...
  const [ocrMethod, setOcrMethod] = useState(typeof (currentDataset as any)?.ocr_method === 'string' ? (currentDataset as any).ocr_method : 'off')
  const {
    ...
    <div className='flex w-full flex-col gap-y-4 px-20 py-8 sm:w-[960px]'>
      ...
      {indexMethod === IndexingType.QUALIFIED && (
        <>
          ...
          <div className={rowClass}>
            <div className={labelClass}>
              <div className='system-sm-semibold text-text-secondary'>{t('datasetSettings.form.ocrMethod')}</div>
            </div>
            <div className='grow'>
              <PureSelect
                options={[
                  { value: 'off', label: 'Default' },
                  { value: 'docling', label: 'SHAI OCR' },
                ]}
                value={ocrMethod}
                onChange={setOcrMethod}
              />
            </div>
          </div>
        </>
