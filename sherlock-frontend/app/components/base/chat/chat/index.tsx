...
const Chat: FC<ChatProps> = ({
  ...
  const prevChatLengthRef = useRef<number>(chatList.length)

  const handleScrollToBottom = useCallback((force = false) => {
  ...
  // Force scroll to bottom when a new message is appended
  useEffect(() => {
    if (chatList.length > prevChatLengthRef.current)
      handleScrollToBottom(true)

    prevChatLengthRef.current = chatList.length
  }, [chatList.length, handleScrollToBottom])
    ...
    if (chatFooterRef.current && chatContainerRef.current) {
      ...
      const resizeContainerObserver = new ResizeObserver((entries) => {
        for (const entry of entries) {
          ...
          chatContainerRef.current!.style.paddingBottom = `${blockSize + 40}px`
          handleScrollToBottom()
        }
      })
    ...
    >
      <div className='relative h-full'>
        ...
        >
          ...
          {
            hasTryToAsk && (
              <div className={cn('mx-auto w-full max-w-[768px] px-4 pt-6')}>
                <TryToAsk
