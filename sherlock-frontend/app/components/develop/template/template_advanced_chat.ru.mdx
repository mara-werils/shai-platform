...
# API расширенного чат-приложения
  ...
  <Col>
    ...
    <Properties>
      ...
      <Property name='inputs' type='object' key='inputs'>
          ...
          Если переменная имеет тип файла, укажите объект с ключами, описанными в `files` ниже.
          По умолчанию `{}`
      </Property>
      ...
      <Property name='files' type='array[object]' key='files'>
          Список файлов, подходит для ввода файлов в сочетании с пониманием текста и ответами на вопросы, доступно только когда модель поддерживает возможность Vision.
          - `type` (string) Поддерживаемый тип: 
            - `document` ('TXT', 'MD', 'MARKDOWN', 'PDF', 'HTML', 'XLSX', 'XLS', 'DOCX', 'CSV', 'EML', 'MSG', 'PPTX', 'PPT', 'XML', 'EPUB')
            - `image` ('JPG', 'JPEG', 'PNG', 'GIF', 'WEBP', 'SVG')
            - `audio` ('MP3', 'M4A', 'WAV', 'WEBM', 'AMR')
            - `video` ('MP4', 'MOV', 'MPEG', 'MPGA')
            - `custom` (Другие типы файлов)
          - `transfer_method` (string) Способ передачи, `remote_url` для URL файла / `local_file` для загрузки файла
          - `url` (string) URL файла (когда способ передачи - `remote_url`)
    ...
    - `event: message_replace` Событие замены содержимого сообщения.
      ...
      - `created_at` (int) Временная метка создания, например, 1705395332
    - `event: workflow_started` workflow начинает выполнение
      - `task_id` (string) ID задачи, используется для отслеживания запроса и API остановки генерации ниже
      - `workflow_run_id` (string) Уникальный ID выполнения workflow
      - `event` (string) фиксирован как `workflow_started`
      - `data` (object) детали
        - `id` (string) Уникальный ID выполнения workflow
        - `workflow_id` (string) ID связанного workflow
        - `sequence_number` (int) Автоинкрементный серийный номер, автоинкремент в приложении, начиная с 1
        - `created_at` (timestamp) Временная метка создания, например, 1705395332
    - `event: node_started` выполнение узла началось
      - `task_id` (string) ID задачи, используется для отслеживания запроса и API остановки генерации ниже
      - `workflow_run_id` (string) Уникальный ID выполнения workflow
      - `event` (string) фиксирован как `node_started`
      - `data` (object) детали
        - `id` (string) Уникальный ID выполнения workflow
        - `node_id` (string) ID узла
        - `node_type` (string) тип узла
        - `title` (string) имя узла
        - `index` (int) Порядковый номер выполнения, используется для отображения последовательности узлов трассировки
        - `predecessor_node_id` (string) необязательно ID узла-предшественника, используется для отображения пути выполнения на холсте
        - `inputs` (object) Содержимое всех переменных предшествующих узлов, используемых в узле
        - `created_at` (timestamp) временная метка начала, например, 1705395332
    - `event: node_finished` выполнение узла завершено, успех или неудача в разных состояниях в одном событии
      - `task_id` (string) ID задачи, используется для отслеживания запроса и API остановки генерации ниже
      - `workflow_run_id` (string) Уникальный ID выполнения workflow
      - `event` (string) фиксирован как `node_finished`
      - `data` (object) детали
        - `id` (string) Уникальный ID выполнения workflow
        - `node_id` (string) ID узла
        - `node_type` (string) тип узла
        ...
        - `predecessor_node_id` (string) необязательно ID узла-предшественника, используется для отображения пути выполнения на холсте
        - `inputs` (object) Содержимое всех переменных предшествующих узлов, используемых в узле
        - `process_data` (json) Необязательные данные процесса узла
        - `outputs` (json) Необязательное содержимое вывода
        - `status` (string) статус выполнения, `running` / `succeeded` / `failed` / `stopped`
        - `error` (string) Необязательная причина ошибки
        - `elapsed_time` (float) Необязательное общее количество секунд, использованное
        - `execution_metadata` (json) метаданные
          - `total_tokens` (int) необязательно использованные токены
          - `total_price` (decimal) необязательно Общая стоимость
          - `currency` (string) необязательно например `USD` / `RMB`
        - `created_at` (timestamp) временная метка начала, например, 1705395332
    - `event: workflow_finished` выполнение workflow завершено, успех или неудача в разных состояниях в одном событии
      - `task_id` (string) ID задачи, используется для отслеживания запроса и API остановки генерации ниже
      - `workflow_run_id` (string) Уникальный ID выполнения workflow
      - `event` (string) фиксирован как `workflow_finished`
      - `data` (object) детали
        - `id` (string) ID выполнения workflow
        - `workflow_id` (string) ID связанного workflow
        - `status` (string) статус выполнения, `running` / `succeeded` / `failed` / `stopped`
        - `outputs` (json) Необязательное содержимое вывода
        - `error` (string) Необязательная причина ошибки
        - `elapsed_time` (float) Необязательное общее количество секунд, использованное
        - `total_tokens` (int) Необязательно использованные токены
        - `total_steps` (int) по умолчанию 0
        - `created_at` (timestamp) время начала
...
<Row>
  <Col>
    ...
    <Properties>
      <Property name='message_id' type='str' key='message_id'>
        For text messages generated by Shai, simply pass the generated message-id directly. The backend will use the message-id to look up the corresponding content and synthesize the voice information directly. If both message_id and text are provided simultaneously, the message_id is given priority.
      </Property>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Запрос" tag="POST" label="/text-to-audio" targetCode={`curl -o text-to-audio.mp3 -X POST '${props.appDetail.api_base_url}/text-to-audio' \\\n--header 'Authorization: Bearer {api_key}' \\\n--header 'Content-Type: application/json' \\\n--data-raw '{\n    "message_id": "5ad4cb98-f0c7-4085-b384-88c403be6290",\n    "text": "Hello Shai",\n    "user": "abc-123"\n}'`}>
