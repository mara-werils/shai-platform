...
import { isBlocked, addFailedAttempt, clearAttempts } from '@/utils/login-lock'
...
export default function MailAndPasswordAuth({ isInvite, isEmailSetup, allowRegistration: _allowRegistration }: MailAndPasswordAuthProps) {
  ...
  const handleEmailPasswordLogin = async () => {
    ...
    if (password.trim().length < 8) {
      Toast.notify({ type: 'error', message: t('login.error.passwordLengthInValid') })
      return
    }
    ...
    try {
      ...
      else if ((error as ResponseError).code === 'authentication_failed') {
        Toast.notify({
          type: 'error',
