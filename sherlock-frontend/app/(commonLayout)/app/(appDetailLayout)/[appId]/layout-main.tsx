...
const AppDetailLayout: FC<IAppDetailLayoutProps> = (props) => {
  ...
  const { isCurrentWorkspaceEditor, isLoadingCurrentWorkspace, currentWorkspace, isNormal } = useAppContext()
  ...
  const getNavigationConfig = useCallback((appId: string, isCurrentWorkspaceEditor: boolean, mode: AppModeEnum) => {
    const navConfig = [
      ...
      ...(isCurrentWorkspaceEditor
        ? [{
          name: t('common.appMenus.scheduler'),
          href: `/app/${appId}/cron`,
          icon: RiTimeLine,
          selectedIcon: RiTimeFill,
        }]
        : []
      ),
      // hide overview if workspace role is normal
      ...((isNormal) ? [] : [{
      ...
      ...(isCurrentWorkspaceEditor
        ? [{
          name: t('common.appMenus.settings'),
          href: `/app/${appId}/settings`,
          icon: RiEqualizer2Line,
          selectedIcon: RiEqualizer2Line,
        }]
        : []
      ),
    ]
  ...
  useEffect(() => {
    ...
    if (!canIEditApp && (pathname.endsWith('configuration') || pathname.endsWith('workflow') || pathname.endsWith('logs') || pathname.endsWith('cron'))) {
