...
import { API_PREFIX, APP_VERSION, CSRF_COOKIE_NAME, CSRF_HEADER_NAME, IS_MARKETPLACE, MARKETPLACE_API_PREFIX, PASSPORT_HEADER_NAME, PUBLIC_API_PREFIX, SHAI_MARKETPLACE_API_PREFIX, WEB_APP_SHARE_CODE_HEADER_NAME } from '@/config'
...
async function base<T>(url: string, options: FetchOptionType = {}, otherOptions: IOtherOptions = {}): Promise<T> {
  ...
  let base: string
  if (isShaiMarketplaceAPI)
    base = SHAI_MARKETPLACE_API_PREFIX
  ...
  if (!isMarketplaceAPI && !isShaiMarketplaceAPI)
  ...
  const res = await client(fetchPathname, {
    ...init,
    headers,
    credentials: (isMarketplaceAPI || isShaiMarketplaceAPI)
      ? 'omit'
