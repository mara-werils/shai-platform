...
export default function AccountSetting({
  ...
  const [loginHistoryUserId, setLoginHistoryUserId] = useState<string | undefined>()
  useEffect(() => {
    if (activeTab === 'login-history') {
      setActiveMenu('login-history')
    } else {
  ...
  const shouldUseFullWidth = isMainUser || currentWorkspace?.role === 'owner' || currentWorkspace?.role === 'admin'
  ...
  const workplaceGroupItems: GroupItem[] = (() => {
    ...
    if (currentWorkspace.role === 'normal') {
      return items.filter(item =>
        item.key !== ACCOUNT_SETTING_TAB.PROVIDER &&
        item.key !== ACCOUNT_SETTING_TAB.DATA_SOURCE
      )
    }
  ...
  return (
    <MenuDialog
      ...
      onClose={onCancel}
    >
      <div className={cn('mx-auto flex h-[100vh] justify-center', shouldUseFullWidth ? 'max-w-[90%] w-[1400px]' : 'max-w-[1048px]')}>
        ...
        <div className={cn('relative flex', shouldUseFullWidth ? 'min-w-[824px] max-w-[100%]' : 'w-[824px]')}>
          ...
          <div ref={scrollRef} className='w-full overflow-y-auto bg-components-panel-bg pb-4'>
            ...
            <div className='px-4 pt-2 sm:px-8'>
              ...
              {activeMenu === 'members' && <MembersPage onOpenLoginHistory={(userId) => {
                setLoginHistoryUserId(userId)
                setActiveMenu('login-history')
              }} />}
              {activeMenu === 'login-history' && <LoginHistoryPage userId={loginHistoryUserId} onBack={() => setActiveMenu('members')} />}
