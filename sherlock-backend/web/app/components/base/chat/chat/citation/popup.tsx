...
import { fetchDocumentUrl } from "@/service/shai/document";
...
const Popup: FC<PopupProps> = ({
  ...
  const downloadDocument = async () => {
    try {
      const { link } = await fetchDocumentUrl(data.documentId)
        ...
        <div className='max-w-[360px] rounded-xl bg-background-section-burn shadow-lg'>
          <div className='px-4 pb-2 pt-3'>
            <div className='flex h-[18px] items-center'>
              ...
              <button className='ml-1 text-text-tertiary' onClick={downloadDocument}><Download02 className='h-4 w-4 shrink-0' /></button>
            </div>
          </div>
