...
export type PluginPageProps = {
  ...
  marketplaceShai?: React.ReactNode
}
const PluginPage = ({
  plugins,
  marketplace,
    ...
    >
      <div
        className={cn(
          ...
          // для маркетплейса и Marketplace SHAI фон должен быть как у body
          (!isPluginsTab) && 'bg-background-body',
        )}
      >
        <div className='flex w-full items-center justify-between'>
          ...
          <div className='flex shrink-0 items-center gap-1'>
            ...
            <PluginTasks />
            {canManagement && checkRoleAccess(['owner', 'admin', 'editor']) && (
              <InstallPluginDropdown
                onSwitchToMarketplaceTab={() => setActiveTab(PLUGIN_PAGE_TABS_MAP.marketplaceShai)}
              />
            )}
            {
              canDebugger && checkRoleAccess(['owner', 'admin', 'editor']) && (
                <DebugInfo />
              )
            }
            {
      ...
      {isPluginsTab && (
        <>
          {plugins}
          {/* TODO: Drag & drop для установки плагинов - временно отключено
          {dragging && (
            <div
      ...
      {activeTab === PLUGIN_PAGE_TABS_MAP.marketplace && enable_marketplace && marketplace}
      {activeTab === PLUGIN_PAGE_TABS_MAP.marketplaceShai && marketplaceShai}
