...
class Account(UserMixin, TypeBase):
    ...
    is_superuser: Mapped[bool] = mapped_column(sa.Boolean, server_default=sa.text("false"), default=False)
    ...
    password_updated_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True, default=None)
...
class Tenant(TypeBase):
    ...
    id: Mapped[str] = mapped_column(StringUUID, server_default=sa.text("uuid_generate_v4()"), init=False)
    name: Mapped[str] = mapped_column(String(255))
    compliance_ai_enabled: Mapped[bool] = mapped_column(sa.Boolean, server_default=sa.text("false"))
    ...
    tariff_id: Mapped[str | None] = mapped_column(StringUUID, default=None)
    created_at: Mapped[datetime] = mapped_column(
        DateTime, server_default=func.current_timestamp(), nullable=False, init=False
    )
    updated_at: Mapped[datetime] = mapped_column(
    ...
    def get_tariff(self):
        return db.session.query(Tariff).filter(Tariff.id == self.tariff_id).first()

    def get_admins_count(self):
        return (db.session.query(TenantAccountJoin).
                filter(TenantAccountJoin.tenant_id == self.id,
                       TenantAccountJoin.role == TenantAccountRole.ADMIN).count())
...
class Tariff(TypeBase):
    __tablename__ = "tariffs"
    __table_args__ = (db.PrimaryKeyConstraint("id", name="tariff_pkey"),)

    id = db.Column(StringUUID, server_default=db.text("uuid_generate_v4()"))
    name = db.Column(db.String(255), nullable=False)
    max_users = db.Column(db.Integer, nullable=False)
    max_admins = db.Column(db.Integer, nullable=False)
    max_vs_space = db.Column(db.Integer, nullable=True)
    bulk_upload_enabled = db.Column(db.Boolean, nullable=False, server_default=db.text("false"))
...
class AccountIntegrate(TypeBase):
    ...
    is_superuser: Mapped[bool] = mapped_column(sa.Boolean, server_default=sa.text("false"))
...
class LoginLog(TypeBase):
    __tablename__ = "login_logs"
    __table_args__ = (
        db.PrimaryKeyConstraint("id", name="login_logs_pkey"),
    )
    id: Mapped[str] = mapped_column(StringUUID, server_default=sa.text("uuid_generate_v4()"), init=False)
    account_id: Mapped[str] = mapped_column(StringUUID, nullable=False)
    ip_address: Mapped[str] = mapped_column(String(64), nullable=False)
    user_agent: Mapped[str] = mapped_column(String(64), nullable=False)
    is_successful: Mapped[bool] = mapped_column(sa.Boolean, server_default=sa.text("true"), default=True)
    created_at: Mapped[datetime] = mapped_column(
