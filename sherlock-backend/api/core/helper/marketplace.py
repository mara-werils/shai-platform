...
def get_plugin_pkg_url(plugin_unique_identifier: str, use_shai_marketplace: bool = False) -> URL:
    if use_shai_marketplace:
        return (URL(str(dify_config.SHAI_MARKETPLACE_API_URL)) / "v1/extension/download/by-identifier").with_query(
...
def download_plugin_pkg(plugin_unique_identifier: str, use_shai_marketplace: bool = False):
    return download_with_size_limit(get_plugin_pkg_url(plugin_unique_identifier, use_shai_marketplace), dify_config.PLUGIN_MAX_PACKAGE_SIZE)
