...
def _extract_text_ocr(file_content: bytes, file_extension: str, *, langs: Optional[list[str]] = None) -> str:
    """
    Sends the file to the external OCR endpoint configured in dify_config.OCR_API_URL.
    Returns joined text or empty string on failure (no exceptions bubble up).
    """
    from core.rag.extractor.ocr_extractor import OcrExtractor

    if not getattr(dify_config, "OCR_API_URL", None):
        raise TextExtractionError("OCR_API_URL must be set to use OCR")

    # Excel (.xls, .xlsx) also allowed here
    allowed_ext = {
        ".pdf",
        ".png",
        ".jpg",
    ...
    if ext not in allowed_ext:
        raise UnsupportedFileTypeError(f"OCR is not supported for extension: {ext}")
    ...
    with tempfile.NamedTemporaryFile(suffix=ext, delete=True) as tmp:
        ...
        extractor = OcrExtractor(
            ocr_api_url=dify_config.OCR_API_URL,   # e.g. "http://34.51.224.29:8000"
            file_path=tmp.name,
            file_extension=ext,
            timeout_seconds=getattr(dify_config, "OCR_TIMEOUT", 60) or 60,
        )
        documents = extractor.extract()
        text = "\n\n".join(
            doc.page_content for doc in documents if getattr(doc, "page_content", None)
        )
        return text.strip()


def _extract_text_from_file(file: File, use_ocr: bool = False) -> str:
    file_content = _download_file_content(file)

    # if caller forced OCR path
    if use_ocr:
        if not file.extension:
            raise UnsupportedFileTypeError("Unable to determine file extension is missing")
        return _extract_text_ocr(file_content=file_content, file_extension=file.extension)

    # primary path by extension or MIME
    if file.extension:
    ...
    # OCR fallback ONLY for scanned PDFs/images (Excel handled inside _extract_text_from_excel)
    ocr_exts = {
        ".pdf",
        ".png",
        ".jpg",
    ...
    if (not text or len(text) < 20) and (file.extension or "").lower() in ocr_exts:
        try:
            ocr_text = _extract_text_ocr(file_content=file_content, file_extension=file.extension)
            if ocr_text:
                return ocr_text
