...
class MarketplaceConfig(BaseSettings):
    ...
    SHAI_MARKETPLACE_API_URL: HttpUrl = Field(
        description="Shai Marketplace API URL",
        default=HttpUrl("https://marketplace.shai.pro"),
    )
...
class AuthConfig(BaseSettings):
    ...
    SESSION_IDLE_TIMEOUT_MINUTES: PositiveInt = Field(
        description="Session timeout in minutes for user inactivity. Session will be invalidated if user is inactive for this duration.",
        default=15,
    )
    LAST_ACTIVE_AT_UPDATE_WINDOW_MINS: PositiveInt = Field(
        description="Window for update last active at time",
        default=5,
    )
...
class RagEtlConfig(BaseSettings):
    ...
    OCR_API_URL: str | None = Field(
        description="OCR API URL, includes various types of OCR models.",
        default=None,
    )
