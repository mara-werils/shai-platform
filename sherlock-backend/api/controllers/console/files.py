...
@console_ns.route("/files/<uuid:file_id>/link")
...
class DocumentLinkApi(Resource):
    @setup_required
    @login_required
    @account_initialization_required
    def get(self, document_id):
        document_id = str(document_id)
        doc = services.dataset_service.DocumentService.get_document_by_id(document_id)
        if not doc:
            return {"error": "Document not found"}, 400
        return {"link": doc.download_path}
