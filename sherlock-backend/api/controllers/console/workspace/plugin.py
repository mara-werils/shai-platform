...
from controllers.console.workspace import plugin_permission_required, roles_required
...
parser_marketplace = (
    reqparse.RequestParser()
    .add_argument("plugin_unique_identifiers", type=list, required=True, location="json")
    .add_argument("use_shai_marketplace", type=bool, required=False, location="json")
)
    ...
    def post(self):
        ...
        try:
            response = PluginService.install_from_marketplace_pkg(
                tenant_id,
                args["plugin_unique_identifiers"],
                use_shai_marketplace=args.get("use_shai_marketplace", False),
            )
        except PluginDaemonClientSideError as e:
            raise ValueError(e)
...
class PluginFetchMarketplacePkgApi(Resource):
    ...
    def get(self):
        ...
        parser.add_argument("use_shai_marketplace", type=bool, required=False, location="args")
...
class PluginUninstallApi(Resource):
    ...
    @account_initialization_required
    # @plugin_permission_required(install_required=True)
    @roles_required([TenantAccountRole.OWNER, TenantAccountRole.ADMIN])
    def post(self):
