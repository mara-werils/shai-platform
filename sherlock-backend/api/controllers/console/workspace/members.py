...
from fields.log_fields import login_log_list_fields
from fields.member_fields import account_with_role_list_fields
from controllers.console.workspace import roles_required
from libs.helper import extract_remote_ip
from libs.login import current_account_with_tenant, login_required
from models.account import Account, TenantAccountRole,AccountStatus, Tenant
from controllers.console.workspace import roles_required
from services.account_service import AccountService, RegisterService, TenantService,  LogsService, PeriodsEnum
...
class MemberListApi(Resource):
    ...
    # @roles_required([TenantAccountRole.OWNER, TenantAccountRole.ADMIN, TenantAccountRole.EDITOR])
    def get(self):
...
@console_ns.route("/workspaces/current/members/<uuid:account_id>/logs")
class LogsApi(Resource):
    ...
    @login_required
    @account_initialization_required
    @marshal_with(login_log_list_fields)
    @roles_required([TenantAccountRole.OWNER, TenantAccountRole.ADMIN, TenantAccountRole.NORMAL, TenantAccountRole.EDITOR])
    def get(self, account_id):
        ...
        parser.add_argument(
            ...
            required=True,
            location="args",
            choices=[p.value for p in PeriodsEnum],
            help="Invalid period. Must be one of: daily, weekly, two_weeks, monthly, three_months, half_year, yearly, all"
        )
        args = parser.parse_args()
        period = PeriodsEnum(args["period"])
        logs = LogsService.get_logs(account_id, period=period)
...
class MemberInviteEmailApi(Resource):
    ...
    def post(self):
        ...
        current_tenant = inviter.current_tenant
        tariff = current_tenant.get_tariff()
        if tariff:
            tenant_accounts = current_tenant.get_accounts()
            if len(invitee_emails) + len(tenant_accounts) > tariff.max_users:
                return {"code": "invalid-invite", "message": f"Too many invites,"
                                                             f" maximum allowed by tariff: {tariff.max_users},"
                                                             f"current members count: {len(tenant_accounts)}"}, 400

            if TenantAccountRole.is_admin_role(invitee_role) and len(
                    invitee_emails) + current_tenant.get_admins_count() > tariff.max_admins:
                return {"code": "invalid-invite", "message":  f"Too many invites for admin users,"
                                                             f" maximum allowed by tariff: {tariff.max_admins},"
                                                             f"current admins count: {current_tenant.get_admins_count()}"}, 400
        ...
        for invitee_email in invitee_emails:
            try:
                ...
                token = RegisterService.invite_new_member(
                    current_tenant, invitee_email, interface_language, role=invitee_role, inviter=inviter
                )
...
@console_ns.route("/workspaces/<uuid:tenant_id>/leave")
class LeaveTenantApi(Resource):
    @setup_required
    @login_required
    @account_initialization_required
    def delete(self, tenant_id):
        if not isinstance(current_user, Account):
            raise Forbidden()
        tenant = db.session.query(Tenant).filter(Tenant.id == str(tenant_id)).first()
        if not tenant:
            raise NotFound("Tenant not found")
        try:
            TenantService.remove_self_from_tenant(tenant, current_user)
        except Exception as e:
...
@console_ns.route("/workspaces/current/members/<uuid:member_id>/update-status")
class MemberUpdateStatusApi(Resource):
    ...
    def put(self, member_id):
        ...
        if not AccountStatus.is_valid_status(new_status):
            return {"code": "invalid-status", "message": "Invalid status"}, 400
        ...
        if (not current_user.id == str(member_id)
        ...
        if member.current_role == TenantAccountRole.OWNER and new_status == AccountStatus.BANNED:
            raise Forbidden('Owner cannot be banned')
        try:
            assert member is not None, "Member not found"
            TenantService.update_member_status(member, new_status)
        except Exception as e:
            raise ValueError(str(e))
