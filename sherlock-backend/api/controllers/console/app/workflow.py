...
from services.app_parameters_service import AppParametersService
...
from services.account_service import AccountAppJoinService
from controllers.console.workspace import app_roles_required
...
class DraftWorkflowApi(Resource):
    ...
    @login_required
    @account_initialization_required
    @app_roles_required([TenantAccountRole.ADMIN, TenantAccountRole.OWNER])
    @get_app_model(mode=[AppMode.ADVANCED_CHAT, AppMode.WORKFLOW])
...
@console_ns.route("/apps/<uuid:app_id>/members")
class AppMembersApi(Resource):
    @api.doc("get_app_members")
    @api.doc(description="Get app members")
    @api.doc(params={"app_id": "Application ID"})
    @api.response(200, "App members retrieved successfully", account_with_role_list_fields)
    @setup_required
    @login_required
    @account_initialization_required
    @marshal_with(account_with_role_list_fields)
    def get(self, app_id: str):
        members = AccountAppJoinService.get_app_members(app_id)
        return {"result": "success", "accounts": members}, 200

    @api.doc("add_app_member")
    @api.doc(description="Add member to app")
    @api.doc(params={"app_id": "Application ID"})
    @api.expect(
        api.model(
            "AddAppMemberRequest",
    ...
    @api.response(201, "Member added successfully")
    @api.response(403, "Permission denied")
    @setup_required
    @login_required
    @account_initialization_required
    def post(self, app_id: str):
        ...
        try:
            ...
            role = args.get("role")
            AccountAppJoinService.add_app_access(account_id, role, app_id)
            return {"message": "success"}, 201
        except Exception as e:
            return {"message": str(e)}, 500

    @api.doc("update_app_member")
    @api.doc(description="Update app member role")
    @api.doc(params={"app_id": "Application ID"})
    @api.expect(
        api.model(
    ...
    @api.response(200, "Member updated successfully")
    @api.response(403, "Permission denied")
    @setup_required
    @login_required
    @account_initialization_required
    def patch(self, app_id: str):
        ...
        try:
            account_id = args.get("account_id")
            role = args.get("role")
            join = AccountAppJoinService.edit_app_access(account_id, role, app_id)
            return {"account_id": join.account_id,
                    "role": join.role}, 200
        except Exception as e:
            return {"message": str(e)}, 500

    @api.doc("remove_app_member")
    @api.doc(description="Remove member from app")
    @api.doc(params={"app_id": "Application ID"})
    @api.expect(
        api.model(
            "RemoveAppMemberRequest",
            {
                "account_id": fields.String(required=True, description="Account ID"),
            },
        )
    )
    @api.response(204, "Member removed successfully")
    ...
    def delete(self, app_id: str):
        ...
        try:
            account_id = args.get("account_id")
            deleted = AccountAppJoinService.remove_app_access(account_id, app_id)
            if deleted:
...
@console_ns.route("/apps/<uuid:app_id>/members/add_many")
class AppMembersAddMany(Resource):
    @api.doc("add_many_app_members")
    @api.doc(description="Add many members to app")
    @api.doc(params={"app_id": "Application ID"})
    @api.expect(
        api.model(
            "AddManyAppMembersRequest",
            {
                "account_ids": fields.List(fields.String, required=True, description="List of Account IDs"),
            },
        )
    )
    @api.response(200, "Members added successfully")
    @api.response(403, "Permission denied")
    @setup_required
    @login_required
    @account_initialization_required
    def post(self, app_id: str):
        ...
        if (not AccountAppJoinService.is_owner(app_id, current_user.id)
                and not TenantAccountRole.is_privileged_role(role)):
            raise Forbidden('Only owner or admin can add app members')
        content_type = request.headers.get("Content-Type", "")

        if "application/json" in content_type:
            parser = reqparse.RequestParser()
            parser.add_argument("account_ids", type=list, required=True, nullable=False, location="json")
            args = parser.parse_args()
        else:
            abort(415)
        try:
            account_ids = args.get("account_ids")
            AccountAppJoinService.add_many(account_ids, app_id)
...
@console_ns.route("/apps/<uuid:app_id>/parameters")
class AppParametersByIdApi(Resource):
    @api.doc("get_app_parameters")
    @api.doc(description="Get parameters for a specific application")
    @api.doc(params={"app_id": "Application ID"})
    @api.response(200, "App parameters retrieved successfully")
    ...
    def get(self, app_id: str):
        ...
        Returns:
            JSON response containing app parameters
        """
        try:
            app_parameters_service = AppParametersService()
            result = app_parameters_service.get_app_parameters_by_id(
                app_id=app_id,
            ...
            return {"error": f"Failed to get app parameters: {str(e)}"}, 500


@console_ns.route("/apps/<uuid:app_id>/access")
class AppAccessApi(Resource):
    @api.doc("get_app_access")
    @api.doc(description="Get app access type")
    @api.doc(params={"app_id": "Application ID"})
    @api.response(200, "App access retrieved successfully")
    @api.response(404, "App not found")
    @setup_required
    @login_required
    @account_initialization_required
    def get(self, app_id: str):
        ...
        if not app:
            return {"message": "not found"}, 404
        return {"access": app.access_type}, 200

    @api.doc("update_app_access")
    @api.doc(description="Update app access type")
    @api.doc(params={"app_id": "Application ID"})
    @api.expect(
        api.model(
            "UpdateAppAccessRequest",
            {
                "access_type": fields.String(required=True, description="Access type"),
            },
        )
    )
    @api.response(200, "App access updated successfully")
    @api.response(400, "Invalid access type")
    ...
    def post(self, app_id: str):
        ...
        if (not AccountAppJoinService.is_owner(app_id, current_user.id)
                and not TenantAccountRole.is_privileged_role(role)):
            raise Forbidden('Only owner or admin can edit app access')
        content_type = request.headers.get("Content-Type", "")

        if "application/json" in content_type:
            parser = reqparse.RequestParser()
            parser.add_argument("access_type", type=str, required=True, nullable=False, location="json")
            args = parser.parse_args()
        else:
            abort(415)
        try:
            valid_types = [value.value for value in DatasetPermissionEnum]
            access_type = args.get("access_type")
            if access_type not in valid_types:
                return {"message": f"invalid access type, must be in {valid_types}"}, 400
