...
from services.account_service import AccountService, RegisterService, TenantService, LogsService
...
class LoginApi(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            if account:
                LogsService.create_log(account_id=account.id, ip_addr=ip_address, is_successful=False)
        ...
        if AccountService.is_password_expired(account):
            token = AccountService.send_reset_password_email(account=account, email=args["email"],
                                                             language=language)
            # return {"result": "success", "data": token,
            #         "is_password_expired": True}
            raise Forbidden("password is expired")

        LogsService.create_log(account.id, ip_address)
