...
from models.model import AccountAppJoin
...
from models.model import AccountAppJoin, App, DifySetup
...
class AccountService:
    ...
    @staticmethod
    def _set_password_updated_at(account: Account) -> Account:
        account.password_updated_at = datetime.now()
        db.session.commit()
        return account


    @staticmethod
    def is_password_expired(account: Account) -> bool:
        if not account.password_updated_at:
            return False
        period = datetime.now() - timedelta(days=dify_config.PASSWORD_EXPIRY_DAYS)
        return account.password_updated_at < period
    ...
    def load_user(user_id: str) -> None | Account:
        ...
        if account.status == AccountStatus.CLOSED:
            raise Unauthorized("Account is closed.")
        ...
        time_since_last_active = datetime.now(UTC).replace(tzinfo=None) - account.last_active_at
        idle_limit = timedelta(minutes=dify_config.SESSION_IDLE_TIMEOUT_MINUTES)

        if time_since_last_active > idle_limit:
            raise Unauthorized(
                f"Session has been idle for too long({dify_config.SESSION_IDLE_TIMEOUT_MINUTES} minutes).")

        if  time_since_last_active > timedelta(minutes=dify_config.LAST_ACTIVE_AT_UPDATE_WINDOW_MINS):
    ...
    def update_account_password(account, password, new_password):
        ...
        AccountService._set_password_updated_at(account)
        return account
...
class TenantService:
    ...
    def create_tenant(name: str, is_setup: bool | None = False, is_from_dashboard: bool | None = False) -> Tenant:
        ...
        tenant = Tenant(name=name, compliance_ai_enabled=False)
    ...
    @staticmethod
    def get_datasets(tenant: Tenant)-> list[Dataset]:
        return (
            db.session.query(Dataset).filter(Dataset.tenant_id == tenant.id).all()
        )

    @staticmethod
    def get_vector_storage_used(tenant: Tenant):
        return DatasetService.get_idx_space_used(TenantService.get_datasets(tenant))
    ...
    @staticmethod
    def check_valid_role(account: Account, roles: list[TenantAccountRole]) -> bool:
        if account.is_superuser:
            return True
        ta = db.session.query(TenantAccountJoin).filter_by(tenant_id=account.current_tenant_id, account_id=account.id).first()
        if not ta:
            return False
        return ta.role in [role.value for role in roles]
    ...
    @staticmethod
    def remove_self_from_tenant(tenant: Tenant, account: Account) -> None:
        ta = db.session.query(TenantAccountJoin).filter_by(tenant_id=tenant.id, account_id=account.id).first()
        if not ta:
            raise MemberNotInTenantError("Member not in tenant.")

        if ta.role == TenantAccountRole.OWNER:
            raise NoPermissionError("Owner cannot leave.")
    ...
    @staticmethod
    def update_member_status(member: Account, new_status: str) -> None:
        """Update member status"""

        # Update the status of the target member
...
class RegisterService:
    ...
    def setup(cls, email: str, name: str, password: str, ip_address: str, language: str):
        ...
        """
        try:
            if db.session.query(Account).filter(Account.email == email).first():
            ...
            if db.session.query(DifySetup).count() == 0:
...
class AccountAppJoinService:

    @staticmethod
    def check_app_access_by_role(account: Account,
                                  roles: list[TenantAccountRole],
                                  app_id: str) -> bool:
        ta = db.session.query(AccountAppJoin).filter_by(app_id=app_id,
    ...
    @staticmethod
    def add_app_access(account_id: str,
                        role: TenantAccountRole,
                        app_id: str) -> AccountAppJoin:
        ...
        if db.session.query(AccountAppJoin).filter(AccountAppJoin.app_id==app_id,
                                           AccountAppJoin.account_id==account_id).first():
            raise Exception("Access already exists")
        join = AccountAppJoin(app_id=app_id, account_id=acc.id, role=role)
        db.session.add(join)
        db.session.commit()
        return join

    @staticmethod
    def remove_app_access(account_id: str, app_id: str) -> bool:
        join = db.session.query(AccountAppJoin).filter(AccountAppJoin.app_id==app_id,
                                           AccountAppJoin.account_id==account_id).first()
        deleted = False
        if join:
    ...
    @staticmethod
    def is_owner(app_id: str, account_id: str) -> bool:
        app = db.session.query(App).filter(App.created_by==account_id, App.id == app_id).first()
        if app:
            return True
        return False

    @staticmethod
    def edit_app_access(account_id: str,
                  role: TenantAccountRole,
                  app_id: str,) -> AccountAppJoin:
        ...
        if not TenantAccountRole.is_valid_role(role):
            raise Exception(f"Role {role} does not exist")
        join = db.session.query(AccountAppJoin).filter(AccountAppJoin.app_id==app_id,
                                           AccountAppJoin.account_id==acc.id).first()
        if not join:
            raise Exception("Join account does not exist")
        join.role = role
        db.session.commit()
        return join
    ...
    def get_app_members(app_id: str) -> list[Account]:
        query = (
            db.session.query(Account, AccountAppJoin.role)
            .select_from(Account)
            .join(AccountAppJoin, Account.id == AccountAppJoin.account_id)
            .filter(AccountAppJoin.app_id == app_id)
        )

        accounts = []

        for account, app_role in query:
            account.app_role = app_role
            accounts.append(account)

        return accounts

    @staticmethod
    def add_many(account_ids: list[str], app_id: str) -> list[AccountAppJoin]:
        joins = []
        session = db.session
        try:

            session.query(AccountAppJoin).filter(AccountAppJoin.app_id == app_id).delete()
...
class PeriodsEnum(enum.StrEnum):
    DAILY = "daily"
    WEEKLY = "weekly"
    TWO_WEEKS = "two_weeks"
    MONTHLY = "monthly"
    ...
    def is_valid_period(cls, period: str) -> bool:
        return period in {period.value for period in cls}
...
class LogsService:
    ...
    @staticmethod
    def get_logs(account_id: str, period: PeriodsEnum) -> list[LoginLog]:
        """
        Получить логи входа по пользователю за определенный период времени.

        Args:
            account_id: ID пользователя
            period: Период времени из PeriodsEnum

        Returns:
            Список логов входа, отсортированный по дате создания (от новых к старым)
        """
        query = db.session.query(LoginLog).filter(LoginLog.account_id == account_id)

        if period != PeriodsEnum.ALL:
            now = datetime.now(UTC)

            match period:
                 case PeriodsEnum.DAILY:
                    start_date = now - timedelta(days=1)
                 case PeriodsEnum.WEEKLY:
                    start_date = now - timedelta(days=7)
                 case PeriodsEnum.TWO_WEEKS:
                    start_date = now - timedelta(days=14)
                 case PeriodsEnum.MONTHLY:
                    start_date = now - timedelta(days=30)
                 case PeriodsEnum.THREE_MONTHS:
                    start_date = now - timedelta(days=90)
                 case PeriodsEnum.HALF_YEAR:
                    start_date = now - timedelta(days=180)
                 case PeriodsEnum.YEARLY:
        ...
        return query.order_by(LoginLog.created_at.desc()).all()

    @staticmethod
    def create_log(account_id: str, ip_addr: str, is_successful=True) -> LoginLog:
        log = LoginLog(account_id=account_id,
                       user_agent=' ',
                       ip_address=ip_addr,
