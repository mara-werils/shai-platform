...
class WorkflowRunService:
    ...
    @staticmethod
    def check_run_allowed(session: Session, app_id: str, max_runs: int) -> bool:
        today = date.today()
        ...
        stmt = select(func.count()).where(
            and_(
                ...
                WorkflowRun.created_at >= start_of_day,
                WorkflowRun.created_at <= end_of_day,
                WorkflowRun.triggered_from == WorkflowRunTriggeredFrom.APP_RUN.value,
            )
        )

        run_count = session.scalar(stmt)
