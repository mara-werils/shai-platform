...
class AppService:
    ...
    @staticmethod
    def has_app_access(account: Account, app_id: str) -> bool:
        app = db.session.query(App).filter(App.id == app_id).first()
        if not app:
            return False
        access_type = app.access_type
        role_str = TenantService.get_user_role(account, account.current_tenant)
        ...
        if not TenantAccountRole.is_privileged_role(role):
            match access_type:
                case DatasetPermissionEnum.ONLY_ME:
                    if not app.created_by == account.id:
                        return False
                case DatasetPermissionEnum.PARTIAL_TEAM:
                    roles = [TenantAccountRole.ADMIN, TenantAccountRole.OWNER]
                    if not app.created_by == account.id:
                        if not AccountAppJoinService.check_app_access_by_role(account, roles, app_id):
                            return False
        return True
